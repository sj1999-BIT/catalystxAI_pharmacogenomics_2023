...
class DeepTTC:
    ...
    def test(self,datagenerator,model):
        ...
        for i,(v_drug,v_gene,label) in enumerate(datagenerator):
            # print(f'v_drug: {v_drug},v_gene: {v_gene},label: {label}')
    ...
    def train(self, train_drug, train_rna, val_drug, val_rna):
        print(f'train_drug: {len(train_drug)}, train_rna: {len(train_rna)}')
        lr = 1e-4
        decay = 0
        BATCH_SIZE = 64
        train_epoch = 10
        ...
        training_generator = data.DataLoader(data_process_loader(
            np.arange(0, len(train_drug)), train_drug.Label.values, train_drug, train_rna), **params)

        validation_generator = data.DataLoader(data_process_loader(
            np.arange(0, len(val_drug)), val_drug.Label.values, val_drug, val_rna), **params)
        ...
        for epo in range(train_epoch):
            for i, (v_d, v_p, label) in enumerate(training_generator):
                # print(f'v_d: {v_d},v_p: {v_p}, label: {label}')
    ...
    def predict(self, drug_data, rna_data):
        ...
        info = data_process_loader(np.arange(0, len(drug_data)),
...
if __name__ == '__main__':
    ...
    # 切分完成
    ori_traindata, ori_testdata = obj.Getdata.ByCancer(random_seed=1)
    # encoding 完成
    traindata, train_rnadata, testdata, test_rnadata = obj.encode(
        traindata=ori_traindata,
        testdata=ori_testdata)
