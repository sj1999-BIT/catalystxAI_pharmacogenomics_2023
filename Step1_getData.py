...
class GetData():
    ...
    def getRna(self,traindata,testdata):
        ...
        columns_to_remove = ['DATA.908134', 'DATA.908120', 'DATA.908442', 'DATA.1789883']
        target = traindata['COSMIC_ID']

        pop_train_list = []
        for i in range(len(train_rnaid)):
            if train_rnaid[i] in columns_to_remove:
                pop_train_list.append(i)

        print(len(pop_train_list))
        # Use slicing to create a new list without the elements at the specified positions
        train_rnaid = [train_rnaid[i] for i in range(len(train_rnaid)) if i not in pop_train_list]

        pop_test_list = []
        for i in range(len(test_rnaid)):
            if test_rnaid[i] in columns_to_remove:
                pop_test_list.append(i)

        print(len(pop_test_list))

        # Use slicing to create a new list without the elements at the specified positions
        test_rnaid = [test_rnaid[i] for i in range(len(test_rnaid)) if i not in pop_test_list]
        # ######################

        # print(f'train_rnaid debug: {train_rnaid}')
        rnadata =  pd.read_csv(self.rnafile,sep='\t')
        print(f'rnadata debug: ')
        # Iterate through the columns and print column name and values
        for column in rnadata.columns:
            print(f"Column: {column}")
            print(rnadata[column])

        train_rnadata = rnadata[train_rnaid]
        test_rnadata = rnadata[test_rnaid]

        return train_rnadata,test_rnadata, pop_train_list, pop_test_list
